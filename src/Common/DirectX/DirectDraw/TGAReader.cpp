...
void CTGAReader::Open(CArchiveFile *pFile, void (*FinishRGBProc)(CDDrawSurface *, LPBYTE, DWORD, LPVOID), LPVOID pFinishRGBProcParam)
{
  CImageReader::Open(pFile, FinishRGBProc, pFinishRGBProcParam);
...
BOOL CTGAReader::ReadImage(CDDrawSurface *lpSurface)
{
  ...
  if(m_bAlphaChannel){ // -> 32-bit Targa
    ...
    if(g_pDirectDraw->Is32BitMode()){
      ...
      for(i = 0; i < m_dwHeight; i++){
        ...
        for(j = 0; j < m_dwWidth; j++, pos1+=4, pos2+=3){
          lpS[pos1] = pLine[pos2];
          lpS[pos1+1] = pLine[pos2+1];
          lpS[pos1+2] = pLine[pos2+2];
        }
  ...
  if(lpSurface->HasAlphaChannel())
    lpSurface->ReleaseAlphaChannel(TRUE);
