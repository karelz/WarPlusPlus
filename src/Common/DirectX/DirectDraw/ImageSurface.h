...
public:
  ...
  BOOL Create(CArchiveFile File, EFileTypes eFileType = FileType_Auto);
  // sets the image key color
	void SetImageKeyColor(DWORD dwKeyColor);
  // sets the image file
	void SetFile(CArchiveFile File, EFileTypes eFileType = FileType_Auto);
  ...
  // sets the finish RGB callback
  void SetFinishRGBProc(void (*FinishRGBProc)(CDDrawSurface *, LPBYTE, DWORD, LPVOID), LPVOID pParam);
...
private:
  ...
  // pointer to finish RGB procedure
