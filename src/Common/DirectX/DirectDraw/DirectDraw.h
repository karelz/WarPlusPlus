...
public:
  ...
  // Enables/ disables the directdraw
  void Enable(BOOL bEnabled = TRUE){ m_bDisabled = bEnabled ? FALSE : TRUE; }

  // returns TRUE if the direct draw was temporarily disabled
  BOOL Disabled(){ return m_bDisabled; }

  // Returns number of bits per pixel in current video mode
	DWORD GetBPP();
...
private:
  // TRUE - the DirectDraw was temporaly disabled
  //   all surface operations will succes but won't do anything (except some like Lock)
  // FALSE - all is OK (enabled)
  BOOL m_bDisabled;
