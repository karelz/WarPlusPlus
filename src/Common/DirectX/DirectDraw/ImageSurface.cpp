...
//  m_dwKeyColor = 0x00FF00FF; // Violet is default key color
  ...
  m_pFinishRGBProc = NULL;
  m_pFinishRGBProcParam = NULL;
}
...
void CImageSurface::SetFile(CArchiveFile File, EFileTypes eFileType)
{
  CString strFileName;
...
BOOL CImageSurface::Create(CArchiveFile File, EFileTypes eFileType)
{
  SetFile(File, eFileType);
...
BOOL CImageSurface::StartImageReader()
{
  ...
  // Open the file
  m_pImageReader->Open(&m_File, m_pFinishRGBProc, m_pFinishRGBProcParam);
...
void CImageSurface::SetFinishRGBProc(void (*FinishRGBProc)(CDDrawSurface *, LPBYTE, DWORD, LPVOID), LPVOID pParam)
{
  m_pFinishRGBProc = FinishRGBProc;
  m_pFinishRGBProcParam = pParam;
}