...
public:
	...
	virtual void BltStretch(CRect *pRectDest, CDDrawSurface *pSource, CRect *pRectSource);
  ...
  // bReadOnly (opened for reading only)
	virtual BYTE * GetAlphaChannel(BOOL bReadOnly = FALSE);
  // Releases the alpha channel after it's use (by GetAlphaChannel)
  // !!!! Attention !!!!! - this can be called only if the surface is NOT locked
  // bReleaseBuffers - TRUE -> internal buffers will be released (next GetAlphaChannel will create them)
  virtual void ReleaseAlphaChannel(BOOL bReleaseBuffers = FALSE);
  // Clears the alpha channel -> means set it opaque
  // With this you should also call some Fill on the whole surface
  virtual void ClearAlphaChannel();
...
protected:
  ...
  // corrects the surface after blit operation
  virtual void CorrectSurfaceAfterBlit(CRect *pRect);

  // Pointer to the alpha channel memory
	BYTE * m_pAlphaChannel;
#ifdef ALPHA_BLIT_QUICK
  // Pointer to backed up data for alpha channel (during some operations on alpha chanell)
  BYTE * m_pAlphaChannelBackup;
  BOOL m_bReadOnlyAlphaChannel;
#endif
  ...
  // Exclusive lock for the surface
