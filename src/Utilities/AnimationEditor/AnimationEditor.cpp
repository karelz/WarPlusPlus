...
BOOL CAnimationEditorApp::InitInstance()
{
  ...
  m_cEventManager.Create ();

  // Initialize archivation
  {
    char pTemp [ 1024 ];
    VERIFY ( GetTempPath ( 1023, pTemp ) <= 1023 );
    CDataArchive::Initialize ( pTemp );
  }

  {
    char txt[1024];
    GetCurrentDirectory(1024, txt);
    g_strStartupDirectory = txt;
  }

  // Initialize DirectDraw
  CDirectDraw::Init ( NULL );
  m_cPrimarySurface.Create ();
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CAnimationEditorDoc),
		RUNTIME_CLASS(CMainFrame),       // main SDI frame window
		RUNTIME_CLASS(CAnimationEditorView));
...
{
  m_cPrimarySurface.Delete ();
  CDirectDraw::Close ();
  CDataArchive::Done ();

  m_cEventManager.Delete ();
