// EULMapexInstance.cpp: implementation of the CEULMapexInstance class.
//
//////////////////////////////////////////////////////////////////////
...
CEULMapexInstance::CEULMapexInstance()
{
  m_pMapex = NULL;
  m_pNext = NULL;
}
...
BOOL CEULMapexInstance::Create(CEMapex *pMapex, DWORD dwX, DWORD dwY, DWORD dwZ)
{
  ASSERT(pMapex != NULL);
...
BOOL CEULMapexInstance::Create(CArchiveFile file)
{
  if(g_dwMapFileVersion < MAP_FILE_VERSION(1, 0)){
    SULMapexInstanceHeader_Old h;
...
void CEULMapexInstance::SaveToFile(CArchiveFile file)
{
  SULMapexInstanceHeader h;
...
void CEULMapexInstance::Draw(CPoint ptTopLeft, CRect *pRectDest, CDDrawSurface *pSurface)
{
...
void CEULMapexInstance::DrawSelected(CPoint ptTopLeft, CRect *pRectDest, CDDrawSurface *pSurface, CDDrawSurface *pBuffer, CDDrawSurface *pSelectionLayer)
{
  CDDrawSurface *pGraphics;
  if(m_pMapex == NULL) return;
  pGraphics = m_pMapex->GetGraphics()->GetFrame(0);
...
void CEULMapexInstance::DrawLandTypes(CPoint ptTopLeft, CRect *pRectDest, CDDrawSurface *pSurface)
{
  if(m_pMapex == NULL) return;
...
void CEULMapexInstance::DrawLandTypesSelected(CPoint ptTopLeft, CRect *pRectDest, CDDrawSurface *pSurface, CDDrawSurface *pBuffer, CDDrawSurface *pSelectionLayer)
{
  if(m_pMapex == NULL) return;
  CSize size = m_pMapex->GetSize();  
...
void CEULMapexInstance::SetPosition(DWORD dwXPos, DWORD dwYPos)
{
  m_dwX = dwXPos; m_dwY = dwYPos;
}

BOOL CEULMapexInstance::PtInMapex(DWORD dwX, DWORD dwY)
{
...
void CEULMapexInstance::SetZPos(DWORD dwZPos)
{
  m_dwZ = dwZPos;
}
